...
1. The `-t` flag is used to specify the type of key that will be generated.
  1. By default, the `ssh-keygen` command will use `rsa` to generate [RSA](https://en.wikipedia.org/wiki/RSA_(cryptosystem)) key pairs.
  2. You can also use `dsa` for [DSA](https://en.wikipedia.org/wiki/Digital_Signature_Algorithm) key pairs. Due to security concerns, it is not commonly used anymore.
  3. Instead of `dsa`, you could use `ecdsa` to generate [ECDSA](https://en.wikipedia.org/wiki/Elliptic_Curve_Digital_Signature_Algorithm) key pairs. It is considered more secure than its counterpart.
  4. Finally, there is also the `ed25519` option to generate [Ed25519](https://en.wikipedia.org/wiki/EdDSA#Ed25519) key pairs. It is currently considered one of the most secure types of keys. This is also what GitHub will ask you for.
